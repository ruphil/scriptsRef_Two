...
function handleSocket(socket){
    ...
    socket.on('getRowsSheetDataForPivot', function(){
      getRowsSheetDataForPivot(socket);
    });
}

// Socker Helper Functions -------------------------------------------------------------------------------------

function getRowsSheetDataForPivot(socket){
  let ranges = ['flight_logs!A1:BB'];
  google.sheets('v4').spreadsheets.values.batchGet({
    ...
    if(!err){
      socket.emit('handleRowsSheetDataForPivot', response.data.valueRanges);
    }
  });
}

//  Leaflet Tracker Function  --------------------------------------------------------------------------------------
